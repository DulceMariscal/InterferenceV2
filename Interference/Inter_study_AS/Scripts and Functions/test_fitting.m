...
%                 index_first_point=minNOSToRemove(subject,group); %Use precomputed value, the same for both A1 and 
               index_first_point=minNOSToRemove(group,subject); 
...
%             subplot(2,4,ind_subplot)
            ...
            ind_subplot=subplot_offset + subject;
            hold on
            subplot_tight(2,8,ind_subplot,margins) %DULCE
%              subplot(2,8,ind_subplot)
            plot(strideVec(~indnan),cdata(~indnan),[mycolors{cond,1} 'o'])
            
%             noise_info(group,subject,cond,:) =
%             is_bad_subject(cdata,subject); Dulce turn it off
...
%     storePath='C:\Users\salat\OneDrive\Documents\MATLAB\Research\CoreL\Interference\Interf_study_AS\SaveDataForStata\';.
    storePath='C:\Users\dum5\OneDrive\_Shared Drive - Interference Project - Alessandro - Dulce_\Params Files\subjectsData\';
    completePath=[storePath 'ALL_DATA.mat'];
