function             [smoothedCurve, originalCurve, R2adj, R2, RMSE] = my_smoothing2(originalCurve, SEL_SM)

Y0= originalCurve(1);

if strcmpi(SEL_SM,'1_EXP')
    [~, smoothedCurve] = my_exp_fit2(originalCurve,'Increasing','Single');
    npars = 3;
...
elseif strcmpi(SEL_SM,'1_EXP_CONSTR')
    [~, smoothedCurve] = my_exp_fit_constr2(originalCurve,'Increasing','Single',Y0);
    npars = 2;

elseif strcmpi(SEL_SM,'2_EXP')
    [~, smoothedCurve] = my_exp_fit2(originalCurve,'Increasing','Double');
    npars = 5;

elseif strcmpi(SEL_SM,'2_EXP_CONSTR')
    [~, smoothedCurve] = my_exp_fit_constr2(originalCurve,'Increasing','Double',Y0);
    npars = 4;
else
