%% COMPUTE CORRELATION BETWEEN BASELINE VARIAIBLITY AND (LEARNING RATE, ...)
...
load(compFold);
table_variables={'dataPert1','dataPert1-5','dataRate6-30','dataSSL30','fitPert1','fitPert1-5','fitRate6-30','fitSSL30','stridesToAVGpert','group','subject','condition','ID'};
...
% Select relevant indices (In this analysis we are only interested in what happened in A1)
COND_COL=find(strcmp(table_variables,'condition'));
indA1=[outcome_table{:,COND_COL}]==A1;
...
for cpar=1:npars
    %Extract par data
    cpar_data=[outcome_table{indA1,cpar}];
...
for cpar=1:nsp
    ...
    %Extract par data
    cpar_data=[outcome_table{indA1,ind_cpar}];
