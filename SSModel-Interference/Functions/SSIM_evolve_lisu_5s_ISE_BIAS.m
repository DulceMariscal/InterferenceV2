function [z, e, X, Xadd] = SSIM_evolve_lisu_5s_ISE_BIAS(k, input , x0, sigmaNE, maxPA)
%% Does not work properly
ntrials = length(input);

% Parameters extraction
kcell = num2cell(k);
[ab,bb,af,bfet,as,bset,aet,bet,nSTD] = kcell{:};
...
%     Pb = normpdf(em,0,nSTD*sigmaNE)/normpdf(0,0,nSTD*sigmaNE); %Probability that cee are due to the body
    ...
    if e(n)>0
        xfa(n+1) = af*xfa(n)  +   bfet*xpe(n)*em*Pw(n);
    else
        xfa(n+1) = af*xfa(n)  +   bfet*xne(n)*em*Pw(n);
    end
    
    %% Bias on the slow state
    xsa(n+1) = as*xsa(n)  +  bset*(xpe(n) + xne(n));
...
em   = abs(e(end));
Pb = normpdf(em,0,nSTD*sigmaNE)/normpdf(0,0,nSTD*sigmaNE); %Probability that cee are due to the body
Pw(end) = 1-Pb; %Probability that currenttly experienced errors are due to the environment
