function [z, e, X] = SSIM_evolve3(k, input , x0) %FACILITATION ALSO IN THE FAST STATE
...
kcell = num2cell(k);
[af,bf,bfet,bfint,as,bs,bset,bsint,aet,bet] = kcell{:};
...
for n=1:ntrials-1
    ...
    if e(n)>0
        xf(n+1) = xf(n+1) + bfet*xpe(n)*em + bfint*xne(n)*em;
    elseif e(n)<0
        xf(n+1) = xf(n+1) + bfet*xne(n)*em + bfint*xpe(n)*em;
    else
    end
