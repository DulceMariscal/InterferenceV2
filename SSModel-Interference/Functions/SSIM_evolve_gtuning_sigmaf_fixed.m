function [z, e, X] = SSIM_evolve_gtuning_sigmaf_fixed(k, input , x0)
%This version weights the error with a gaussian function
ntrials = length(input);

% Parameters extraction
kcell = num2cell(k);
[af,bf,fpeak,as,bs,speak,sstd,aet,bet] = kcell{:};
fstd = 1/3; %In this way we explicitely set that there is not going ot be interference in the fast state
%Initializations
z = zeros(1,ntrials);
xf = zeros(1,ntrials);
xs = zeros(1,ntrials);
