function [z, e, X, Xadd] = SSIM_evolve_CWSS_ISE(k, input , x0, sigmaNE)
...
for n=1:ntrials-1
    ...
    %On the fast state
    if e(n)>0
        xf(n+1) = xf(n+1) + Pw(n)*bfet*xpe(n)*em;
    elseif e(n)<0
        xf(n+1) = xf(n+1) + Pw(n)*bfet*xne(n)*em;
    else
    end
    ...
    % 2. Update
    if e(n)>0
        xpe(n+1) = xpe(n+1) + Pw(n)*bet*e(n);
    elseif e(n)<0
        xne(n+1) = xne(n+1) + Pw(n)*bet*e(n);
    end
