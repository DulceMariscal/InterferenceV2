function [z, e, X] = SSIM_evolve_lisu_5s_noFac(k, input , x0) 
...
kcell = num2cell(k);
[af,bf,bfet,as,bs,bsint,aet,bet,ab,bb] = kcell{:};
...
for n=1:ntrials-1
    ...
    %% Primitives update 
     ...
     % 2. Learning %max adapted state is constrained
        if e(n)>0
            xpe(n+1) = min([xpe(n+1) + bet*e(n), 0.345]);
        elseif e(n)<0
            xne(n+1) = max([xne(n+1) + bet*e(n), -0.345]);
        else
        end
end
