function [z, e, X] = SSIM_evolve4(k, input , x0) %MULTIPLICATIVE INTERFERENCE
...
for n=1:ntrials-1
    ...
    if e(n)>0
        xs(n+1) = xs(n+1) + bset*xpe(n)*em;
    elseif e(n)<0
        xs(n+1) = xs(n+1) + bset*xne(n)*em;
    end
    
    %Interference in the slow state
     xs(n+1) = xs(n+1) + bsint*xpe(n)*xne(n)*e(n);
